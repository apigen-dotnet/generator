...
public class ClientGenerator
{
  ...
  private GeneratedFile GenerateFormUrlEncodedContentExtensions()
  {
    ...
    sb.AppendLine("{");
    sb.AppendLine("  /// <summary>");
    sb.AppendLine("  /// Converts a DTO object to FormUrlEncodedContent for OAuth2 token endpoints,");
    sb.AppendLine("  /// the Hetzner Robot Webservice, and any other form-encoded body endpoint.");
    sb.AppendLine("  /// Uses JsonPropertyName attribute for field names. Null properties are omitted.");
    sb.AppendLine("  /// Repeated array fields use PHP-style indexed brackets (name[0]=…&name[1]=…),");
    sb.AppendLine("  /// matching http_build_query() output and what Hetzner's reference clients emit.");
    sb.AppendLine("  /// Numeric and DateTime values are formatted with invariant culture.");
    sb.AppendLine("  /// </summary>");
    ...
    sb.AppendLine();
    sb.AppendLine("      AppendField(fields, fieldName, value);");
    sb.AppendLine("    }");
    sb.AppendLine();
    sb.AppendLine("    return new FormUrlEncodedContent(fields);");
    sb.AppendLine("  }");
    sb.AppendLine();
    sb.AppendLine("  private static void AppendField(List<KeyValuePair<string, string>> fields, string name, object value)");
    sb.AppendLine("  {");
    sb.AppendLine("    switch (value)");
    sb.AppendLine("    {");
    sb.AppendLine("      case string s:");
    sb.AppendLine("        fields.Add(new(name, s));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case bool b:");
    sb.AppendLine("        fields.Add(new(name, b ? \"true\" : \"false\"));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case int or long or short or byte or uint or ulong or ushort or sbyte:");
    sb.AppendLine("        fields.Add(new(name, ((IFormattable)value).ToString(null, CultureInfo.InvariantCulture)));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case float or double or decimal:");
    sb.AppendLine("        fields.Add(new(name, ((IFormattable)value).ToString(null, CultureInfo.InvariantCulture)));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case DateTime dt:");
    sb.AppendLine("        fields.Add(new(name, dt.ToString(\"o\", CultureInfo.InvariantCulture)));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case DateTimeOffset dto:");
    sb.AppendLine("        fields.Add(new(name, dto.ToString(\"o\", CultureInfo.InvariantCulture)));");
    sb.AppendLine("        break;");
    sb.AppendLine("      case IEnumerable enumerable:");
    sb.AppendLine("        // PHP-style bare brackets: `name[]=…&name[]=…`. This matches what");
    sb.AppendLine("        // Hetzner's docs literally show in their curl examples, what PHP's");
    sb.AppendLine("        // http_build_query produces for unindexed arrays, and what the");
    sb.AppendLine("        // PHP/Laravel/Symfony/WordPress/Drupal ecosystems parse natively.");
    sb.AppendLine("        // Equivalent to `name[0]=…&name[1]=…` on any PHP server but closer");
    sb.AppendLine("        // to the documented format.");
    sb.AppendLine("        foreach (object? item in enumerable)");
    sb.AppendLine("        {");
    sb.AppendLine("          if (item == null) continue;");
    sb.AppendLine("          AppendField(fields, $\"{name}[]\", item);");
    sb.AppendLine("        }");
    sb.AppendLine("        break;");
    sb.AppendLine("      default:");
    sb.AppendLine("        fields.Add(new(name, value.ToString() ?? \"\"));");
    sb.AppendLine("        break;");
    sb.AppendLine("    }");
    sb.AppendLine("  }");
    sb.AppendLine("}");
