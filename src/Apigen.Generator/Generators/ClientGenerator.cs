...
public class ClientGenerator
{
  ...
  private GeneratedFile GenerateFormUrlEncodedContentExtensions()
  {
    ...
    sb.AppendLine("        // PHP-style indexed brackets: `name[0]=…&name[1]=…`. Hetzner Robot");
    sb.AppendLine("        // requires this (its PHP reference client emits the same shape).");
    sb.AppendLine("        int i = 0;");
    sb.AppendLine("        foreach (object? item in enumerable)");
    sb.AppendLine("        {");
    sb.AppendLine("          if (item == null) { i++; continue; }");
    sb.AppendLine("          AppendField(fields, $\"{name}[{i}]\", item);");
    sb.AppendLine("          i++;");
    sb.AppendLine("        }");
    sb.AppendLine("        break;");
